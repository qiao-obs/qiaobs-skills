...
**一句话价值：** 当任务已经授权且包含多个相邻阶段时，以更少交接、持续可见的方式把它组织成有边界、可验证的工作包；进度汇报发出后继续，不把用户变成调度员。
...
**低交互降低什么：** 用户不必替 Agent 拆步骤、逐次回复“继续”、选择普通实现细节、复制粘贴子代理结果，或逐命令批准安全且已授权的常规工作。

**低交互不降低什么：** 用户对当前阶段、已完成证据、未验证项、失败改道、风险、授权边界和最终结果的知情权。
...
```text
任务契约 → 可验证工作包 → 按依赖推进
→ CHECKPOINT（发出后继续） → 局部失败诊断/重试
→ 整体验证 → COMPLETE/PARTIAL/BLOCKED
```

**CHECKPOINT 与 GATE：**

- `CHECKPOINT` 是信息性进度更新：说明已完成、正在进行、尚未验证、发布状态和下一步；不等待回复。
- `GATE` 是真实授权/输入门：只用于登录/MFA、权限拒绝、未授权高影响动作、关键事实缺失或无法安全验证；必须明确等待用户。

**会得到什么输出：** 工作包执行板、阶段检查点、失败/重试证据、改变与未改变的文件、分层验证结果、真实最终状态和唯一必要的用户动作。

| 用户不必做 | Codex 必须做 |
| --- | --- |
| 逐步回复“继续” | 主动汇报阶段进度，并在无 GATE 时继续 |
| 选择常规实现细节 | 在证据和授权边界内自行决策 |
| 阅读命令猜状态 | 明确写已完成、进行中、未验证和发布状态 |
| 调度多个子代理 | 说明分工、批次结果和主线程整合 |

**一个真实提示词：**

> 请在当前授权范围内一次完成审计、实现、测试、文档和 Git 收尾；少打扰我，但请持续告诉我做到哪里，每次进度不需要等我回复。保留已有修改，不使用破坏性命令，只在登录/MFA、权限拒绝或未授权高影响动作时停下。

**一个简短输出示例：**

> **CHECKPOINT（不等待回复）：** 已完成基线审计和范围冻结；正在进入实施；静态验证、安装冒烟、CI、PR 和 Release 尚未运行/创建。我会继续执行。若外部服务要求 MFA，再建立 **GATE**，不会把本地通过冒充发布完成。

深入阅读：[用户说明页](docs/skills/run-autonomous-workpacks.zh-CN.md) · [执行入口](skills/run-autonomous-workpacks/SKILL.md) · [通信契约](skills/run-autonomous-workpacks/references/communication-and-progress-contract.md)
...
当前稳定版本为 [`v0.1.2`](https://github.com/qiao-obs/qiaobs-skills/releases/tag/v0.1.2)。v0.2 的精修内容将在完成验证和发布后更新。
